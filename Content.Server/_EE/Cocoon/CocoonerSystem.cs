...
namespace Content.Server._EE.Cocoon
{
    public sealed class CocooningSystem : EntitySystem
    {
        ...
        private void OnDamageChanged(EntityUid uid, CocoonComponent component, DamageChangedEvent args)
        {
            ...
            _damageableSystem.TryChangeDamage((Entity<DamageableComponent?>)component.Victim, damage);
        }
        ...
        private void OnUnCocoonDoAfter(EntityUid uid, CocoonerComponent component, UnCocoonDoAfterEvent args)
        {
            ...
            _adminLogger.Add(LogType.Action, LogImpact.Low, $"{ToPrettyString(args.Args.User):player} uncocooned {ToPrettyString(args.Args.Target.Value):target}");
            args.Handled = true;
        }
    }
}
