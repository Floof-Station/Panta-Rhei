...
public sealed class WeatherSystem : SharedWeatherSystem
{
    ...
    public override void Update(float frameTime)
    {
        ...
        if (player == null)
        {
            // Euph - cancel all sounds in this case
            var query2 = EntityQueryEnumerator<WeatherStatusEffectComponent, StatusEffectComponent>();
            while (query2.MoveNext(out var uid, out var weather, out var status))
            {
        ...
        while (query.MoveNext(out var uid, out var weather, out var status))
        {
            ...
            {
                weather.Stream = _audio.Stop(weather.Stream);
                continue; // Euph - this used to return
            }
